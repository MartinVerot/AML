...
def plot_data(Z):
    ...
    for n in range(1,len(Threshold)):
        ...
        for j in range(Threshold[n]):
            ...
            if k == Z:
                ax2.add_patch(patches.Rectangle((j, n-0.5), 1,1, edgecolor = edgecolor, facecolor = 'red', fill=True))
            elif Z>CumSum[n]:
                ax2.add_patch(patches.Rectangle((j, n-0.5), 1,1, edgecolor = edgecolor, facecolor = '#dddddd', fill=True))
            elif Z<=CumSum[n] and Z>=CumSum[n-1] and j<Z-CumSum[n-1]:
                ax2.add_patch(patches.Rectangle((j, n-0.5), 1,1, edgecolor = edgecolor, facecolor = '#dddddd', fill=True))
            else:
                ax2.add_patch(patches.Rectangle((j, n-0.5), 1,1, edgecolor = edgecolor, facecolor = 'white', fill=True))
    #print(EnIo)
    #Updating the dots
